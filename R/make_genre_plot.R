...
pTop20 <- genreSummary %>%
  arrange(desc(Count)) %>%
  top_n(20) %>%
  ggplot(aes(x = genre_name, y = Count)) +
  geom_bar(stat='identity') +
  geom_text(aes(x = genre_name, y = 10, label = genre_name, hjust = 0), colour = "white", size = 4.5) +
  coord_flip(ylim=c(50,1250)) +
  ...
  ylab('Podcasts') +
  xlab('Top 20 Genres')
ggsave('r_plots/genre_counts_top20.pdf', width = 13, height = 10)